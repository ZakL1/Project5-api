...
class ChallengeListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if status == "ongoing":
            return Challenge.objects.filter(start_date__lte=today, end_date__gte=today).order_by("-start_date")
        elif status == "past":
            return Challenge.objects.filter(end_date__lt=today).order_by("-start_date")
        else:
